...
If you open the dashboard over plain HTTP (e.g. `http://<lan-ip>:18798/` or
`http://<tailscale-ip>:18798/`), the browser runs in a **non-secure context** and
blocks WebCrypto, so device identity can’t be generated.

**Fix:**

- Prefer HTTPS via [Tailscale Serve](/gateway/tailscale).
- Or open locally on the gateway host: `http://127.0.0.1:18798/`.
...
### Address Already in Use (Port 18798)
...
The app connects to a local gateway on port `18798`. If it stays stuck:
...
```bash
lsof -nP -iTCP:18798 -sTCP:LISTEN
```
