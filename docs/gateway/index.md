...
```bash
openclaw gateway --port 18798
# for full debug/trace logs in stdio:
openclaw gateway --port 18798 --verbose
...
- Binds WebSocket control plane to `127.0.0.1:<port>` (default 18798).
...
- Port precedence: `--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > default `18798`.
...
- Tailscale/VPN preferred; otherwise SSH tunnel:
  ...
  ```
- Clients then connect to `ws://127.0.0.1:18798` through the tunnel.
...
[Service]
ExecStart=/usr/local/bin/openclaw gateway --port 18798
Restart=always
RestartSec=5
