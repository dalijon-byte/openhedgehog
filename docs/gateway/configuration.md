...
// ~/.openclaw/openclaw.json
{
  gateway: { port: 18798, auth: { token: "secret" } },
...
- bind: `loopback`
- port: `18798` (single port for WS + HTTP)

```json5
{
  gateway: {
    mode: "local", // or "remote"
    port: 18798, // WS + HTTP multiplex
    bind: "loopback",
...
- Precedence: `--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > default `18798`.
...
- `gateway.remote.transport` selects the macOS remote transport (`ssh` default, `direct` for ws/wss). When `direct`, `gateway.remote.url` must be `ws://` or `wss://`. `ws://host` defaults to port `18798`.
...
```json5
{
  hooks: {
    gmail: {
      ...
      hookUrl: "http://127.0.0.1:18798/hooks/gmail",
      includeBody: true,
      maxBytes: 20000,
