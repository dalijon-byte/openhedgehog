...
The wizard can also build UI assets automatically. After onboarding, you typically run the Gateway on port **18798**.
...
- **Tailnet bind**: run `openclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18798/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18798:127.0.0.1:18798 user@host` then open `http://127.0.0.1:18798/?token=...` from `openclaw dashboard`.
...
4. **Use the tailnet hostname**
   ...
   - Gateway WS: `ws://your-vps.tailnet-xxxx.ts.net:18798`
...
```
--port > OPENCLAW_GATEWAY_PORT > gateway.port > default 18798
```
...
OpenClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18798`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.
...
- If remote, tunnel first: `ssh -N -L 18798:127.0.0.1:18798 user@host` then open `http://127.0.0.1:18798/?token=...`.
...
1. Use the WS URL: `ws://<host>:18798` (or `wss://...` if HTTPS).
...
```
openclaw tui --url ws://<host>:18798 --token <token>
```
