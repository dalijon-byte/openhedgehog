...
- **Localhost**: open `http://127.0.0.1:18798/`. If you see “unauthorized,” run `openclaw dashboard` and use the tokenized link (`?token=...`).
...
- Ensure the gateway is reachable (local: `openclaw status`; remote: SSH tunnel `ssh -N -L 18798:127.0.0.1:18798 user@host` then open `http://127.0.0.1:18798/?token=...`).
