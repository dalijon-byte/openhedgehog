...
declare global {
  interface Window {
    __OPENHEDGEHOG_ASSISTANT_NAME__?: string;
    __OPENHEDGEHOG_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__OPENHEDGEHOG_ASSISTANT_NAME__,
    avatar: window.__OPENHEDGEHOG_ASSISTANT_AVATAR__,
  });
}
