...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openhedgehog/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openhedgehog");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openhedgehog/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENHEDGEHOG_CONTROL_UI_BASE_PATH__ = "/openhedgehog";
    const app = mountApp("/openhedgehog/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openhedgehog");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openhedgehog/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "openhedgehog.control.settings.v1",
