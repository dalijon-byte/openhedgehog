...
declare const __OPENHEDGEHOG_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENHEDGEHOG_VERSION__ === "string" && __OPENHEDGEHOG_VERSION__) ||
  process.env.OPENHEDGEHOG_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureOpenHedgehogCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[openhedgehog] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
