...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    ...
    const shared = path.join(tmpdir(), "openhedgehog-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.openhedgehog/agents/shared/agent" },
                { id: "b", agentDir: "~/.openhedgehog/agents/shared/agent" },
              ],
            },
