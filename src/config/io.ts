...
import type { OpenHedgehogConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareOpenHedgehogVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "OPENHEDGEHOG_GATEWAY_TOKEN",
  "OPENHEDGEHOG_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): OpenHedgehogConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenHedgehogConfig;
}
...
function stampConfigVersion(cfg: OpenHedgehogConfig): OpenHedgehogConfig {
...
function warnIfConfigFromFuture(
  ...
  const cmp = compareOpenHedgehogVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenHedgehog (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpenHedgehogConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenHedgehogConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpenHedgehogConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenHedgehogConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenHedgehogConfig) {
    clearConfigCache();
...
// module scope. `OPENHEDGEHOG_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENHEDGEHOG_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENHEDGEHOG_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenHedgehogConfig {
...
export async function writeConfigFile(cfg: OpenHedgehogConfig): Promise<void> {
  clearConfigCache();
