  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.openhedgehog/logs/openhedgehog.log" },
        hooks: {
          path: "~/.openhedgehog/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.openhedgehog/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".openhedgehog", "logs", "openhedgehog.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".openhedgehog", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".openhedgehog", "telegram.token"),
      );
