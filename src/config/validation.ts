...
import type { OpenHedgehogConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenHedgehogConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenHedgehogConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenHedgehogSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenHedgehogConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenHedgehogConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenHedgehogConfig)),
    ),
  };
}
