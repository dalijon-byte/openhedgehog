...
const ETC_OPENHEDGEHOG_DIR = path.join(ROOT_DIR, "etc", "openhedgehog");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "openhedgehog.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenHedgehogPath(...parts: string[]) {
  return path.join(ETC_OPENHEDGEHOG_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenHedgehogPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "openhedgehog.json"))).toEqual({
      shared: true,
    });
  });
});
  ...
  it("supports modular config structure", () => {
    const files = {
      [configPath("gateway.json")]: {
        gateway: { port: 18798, bind: "loopback" },
      },
