...
const baseUrl = new URL("http://127.0.0.1:18798/hooks/gmail");
...
describe("hooks mapping", () => {
  ...
  it("runs transform module", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-hooks-"));
    ...
    const result = await applyHookMappings(mappings, {
      payload: { name: "Ada" },
      headers: {},
      url: new URL("http://127.0.0.1:18798/hooks/custom"),
      path: "custom",
    });
  ...
  it("treats null transform as a handled skip", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-hooks-skip-"));
    const modPath = path.join(dir, "transform.mjs");
    ...
    const result = await applyHookMappings(mappings, {
      payload: {},
      headers: {},
      url: new URL("http://127.0.0.1:18798/hooks/skip"),
      path: "skip",
    });
  ...
  it("rejects missing message", async () => {
    ...
    const result = await applyHookMappings(mappings, {
      payload: {},
      headers: {},
      url: new URL("http://127.0.0.1:18798/hooks/noop"),
      path: "noop",
    });
