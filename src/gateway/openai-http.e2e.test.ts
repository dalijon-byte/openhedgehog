    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "openhedgehog", messages: [{ role: "user", content: "hi" }] },
          { "x-openhedgehog-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-openhedgehog-agent-id": "beta",
            "x-openhedgehog-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
