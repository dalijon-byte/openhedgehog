...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENHEDGEHOG_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENHEDGEHOG_SKIP_GMAIL_WATCHER;
    process.env.OPENHEDGEHOG_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENHEDGEHOG_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENHEDGEHOG_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENHEDGEHOG_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
