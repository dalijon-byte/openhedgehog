...
import type { OpenHedgehogConfig, ModelProviderConfig } from "../config/types.js";
import { resolveOpenHedgehogAgentDir } from "../agents/agent-paths.js";
...
import { ensureOpenHedgehogModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPENHEDGEHOG_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPENHEDGEHOG_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPENHEDGEHOG_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: OpenHedgehogConfig;
  agentDir: string;
}): OpenHedgehogConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<OpenHedgehogConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.OPENHEDGEHOG_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OPENHEDGEHOG_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOpenHedgehogAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-live-state-"));
  process.env.OPENHEDGEHOG_STATE_DIR = tempStateDir;
  ...
  process.env.OPENHEDGEHOG_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.openhedgehog-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOpenHedgehogAgentDir();
  const sanitizedCfg: OpenHedgehogConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "OpenHedgehog live tool probe (local, safe): " +
    ...
    process.env.OPENHEDGEHOG_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPENHEDGEHOG_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOpenHedgehogModelsJson(cfg);
      ...
      const rawModels = process.env.OPENHEDGEHOG_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.openhedgehog-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
