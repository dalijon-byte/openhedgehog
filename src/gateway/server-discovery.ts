...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/openhedgehog/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (OpenHedgehog)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.OPENHEDGEHOG_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "openhedgehog");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.OPENHEDGEHOG_TAILNET_DNS?.trim();
