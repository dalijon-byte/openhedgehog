...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openhedgehog-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openhedgehog-account-id")?.trim() || undefined;
  ...
  const allTools = createOpenHedgehogTools({
