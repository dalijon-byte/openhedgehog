...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openhedgehog-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENHEDGEHOG_CONFIG_PATH = path.join(root, "openhedgehog.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openhedgehog.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "openhedgehog-gateway-test"),
        ...fileDefaults,
