...
export function resolveHooksConfig(cfg: OpenHedgehogConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-openhedgehog-token"] === "string"
      ? req.headers["x-openhedgehog-token"].trim()
      : "";
  if (headerToken) {
