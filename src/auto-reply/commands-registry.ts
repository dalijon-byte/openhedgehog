...
import type { OpenHedgehogConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OpenHedgehogConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenHedgehogConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenHedgehogConfig),
...
export function getCommandDetection(_cfg?: OpenHedgehogConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenHedgehogConfig) {
