...
const requireAgentDir = () => {
  const agentDir = process.env.OPENHEDGEHOG_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENHEDGEHOG_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENHEDGEHOG_STATE_DIR;
  const previousAgentDir = process.env.OPENHEDGEHOG_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPENHEDGEHOG_AGENT_DIR;
    } else {
      process.env.OPENHEDGEHOG_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENHEDGEHOG_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENHEDGEHOG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-oauth-"));
    process.env.OPENHEDGEHOG_STATE_DIR = tempStateDir;
    process.env.OPENHEDGEHOG_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENHEDGEHOG_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENHEDGEHOG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-minimax-"));
    process.env.OPENHEDGEHOG_STATE_DIR = tempStateDir;
    process.env.OPENHEDGEHOG_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENHEDGEHOG_AGENT_DIR;
