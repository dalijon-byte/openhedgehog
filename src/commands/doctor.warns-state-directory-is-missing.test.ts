...
beforeEach(() => {
  ...
  resolveOpenHedgehogPackageRoot.mockReset().mockResolvedValue(null);
  ...
  resolveGatewayProgramArguments.mockReset().mockResolvedValue({
    programArguments: ["node", "cli", "gateway", "--port", "18798"],
  });
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENHEDGEHOG_STATE_DIR;
  originalUpdateInProgress = process.env.OPENHEDGEHOG_UPDATE_IN_PROGRESS;
  process.env.OPENHEDGEHOG_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENHEDGEHOG_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENHEDGEHOG_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENHEDGEHOG_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenHedgehogPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenHedgehogPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/openhedgehog.json",
    createConfigIO,
...
vi.mock("../infra/openhedgehog-root.js", () => ({
  resolveOpenHedgehogPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENHEDGEHOG_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENHEDGEHOG_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openhedgehog.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENHEDGEHOG_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
