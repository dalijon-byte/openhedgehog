...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenHedgehog status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openhedgehog security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openhedgehog security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openhedgehog.ai/faq");
  runtime.log("Troubleshooting: https://docs.openhedgehog.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openhedgehog status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openhedgehog logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openhedgehog status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openhedgehog gateway probe")}`);
  }
}
