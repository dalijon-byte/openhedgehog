...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenHedgehogAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openhedgehog models auth setup-token")}\` or \`${formatCliCommand("openhedgehog configure")}\`.`
          : `Run \`${formatCliCommand("openhedgehog configure")}\` or set an API key env var.`;
