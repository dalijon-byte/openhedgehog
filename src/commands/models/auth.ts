...
import { readConfigFileSnapshot, type OpenHedgehogConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenHedgehogConfig, model: string): OpenHedgehogConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openhedgehog plugins install")}\`.`,
    );
  }
