...
export async function updateConfig(
  mutator: (cfg: OpenHedgehogConfig) => OpenHedgehogConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenHedgehogConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenHedgehogConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openhedgehog agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
