...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenHedgehogAgentDir: vi.fn().mockReturnValue("/tmp/openhedgehog-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openhedgehog-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openhedgehog-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenHedgehogAgentDir: mocks.resolveOpenHedgehogAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenHedgehogAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openhedgehog-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openhedgehog-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openhedgehog-agent-custom");
