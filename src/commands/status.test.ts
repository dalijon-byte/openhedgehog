...
beforeAll(() => {
  previousProfile = process.env.OPENHEDGEHOG_PROFILE;
  process.env.OPENHEDGEHOG_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENHEDGEHOG_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openhedgehog-root.js", () => ({
  resolveOpenHedgehogPackageRoot: vi.fn().mockResolvedValue("/tmp/openhedgehog"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openhedgehog/pnpm-lock.yaml",
      markerPath: "/tmp/openhedgehog/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenHedgehog status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openhedgehog status --all") ||
          l.includes("openhedgehog --profile isolated status --all") ||
          l.includes("openhedgehog status --all") ||
          l.includes("openhedgehog --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENHEDGEHOG_GATEWAY_TOKEN = "abcd1234";
    try {
