...
function stripUnknownConfigKeys(config: OpenHedgehogConfig): {
  config: OpenHedgehogConfig;
  removed: string[];
} {
  const parsed = OpenHedgehogSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OpenHedgehogConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".openhedgehog");
  const targetPath = path.join(targetDir, "openhedgehog.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OpenHedgehogConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openhedgehog doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openhedgehog doctor --fix")}" to apply these changes.`,
      );
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "openhedgehog doctor --fix" to remove these keys.');
    }
  }
