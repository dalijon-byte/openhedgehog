...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENHEDGEHOG_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENHEDGEHOG_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENHEDGEHOG_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENHEDGEHOG_GATEWAY_PASSWORD");
  });
