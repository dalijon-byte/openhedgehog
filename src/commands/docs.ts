...
const SEARCH_TOOL = "https://docs.openhedgehog.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openhedgehog.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(
        `${theme.muted("Search:")} ${formatCliCommand('openhedgehog docs "your query"')}`,
      );
    } else {
      runtime.log("Docs: https://docs.openhedgehog.ai/");
      runtime.log(`Search: ${formatCliCommand('openhedgehog docs "your query"')}`);
    }
    return;
  }
