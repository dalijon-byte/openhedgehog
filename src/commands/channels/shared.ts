...
import { type OpenHedgehogConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
...
): Promise<OpenHedgehogConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("openhedgehog doctor")}.`);
    runtime.exit(1);
    return null;
  }
