...
import type { OpenHedgehogConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenHedgehogConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openhedgehog security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openhedgehog configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openhedgehog config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openhedgehog doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "openhedgehog config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("openhedgehog config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
