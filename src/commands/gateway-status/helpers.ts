import type { OpenHedgehogConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.OPENHEDGEHOG_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.OPENHEDGEHOG_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: OpenHedgehogConfig) {
