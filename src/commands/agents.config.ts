...
export function listAgentEntries(cfg: OpenHedgehogConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenHedgehogConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenHedgehogConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenHedgehogConfig): AgentSummary[] {
