...
function resolveMode(cfg: OpenHedgehogConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenHedgehog doctor");

  const root = await resolveOpenHedgehogPackageRoot({
  ...
  let cfg: OpenHedgehogConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openhedgehog configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openhedgehog config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openhedgehog setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openhedgehog doctor --fix")}" to apply changes.`);
  }
