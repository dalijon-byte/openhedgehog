...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints JSON", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (OpenHedgehog)",
        displayName: "Studio",
        domain: "local.",
        host: "studio.local",
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        ...
        host: "studio.openhedgehog.internal",
        lanHost: "studio.local",
        tailnetDns: "studio.tailnet.ts.net",
        gatewayPort: 18798,
        sshPort: 22,
    ...
    expect(out).toContain("- Studio openhedgehog.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.openhedgehog.internal");
    expect(out).toContain("  ws: ws://studio.tailnet.ts.net:18798");
  });
  ...
  it("validates gateway ports and handles force/start errors", async () => {
    ...
    await expect(
      programForceFail.parseAsync(
        ["gateway", "--port", "18798", "--token", "test-token", "--force", "--allow-unconfigured"],
        { from: "user" },
      ),
    ).rejects.toThrow("__exit__:1");
    ...
    await expect(
      programStartFail.parseAsync(
        ["gateway", "--port", "18798", "--token", "test-token", "--allow-unconfigured"],
        {
          from: "user",
        },
      ),
