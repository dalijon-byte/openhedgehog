...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(18798)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(18798)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(18798);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(18798, {
      timeoutMs: 500,
      intervalMs: 100,
