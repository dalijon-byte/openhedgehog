...
const resolveGatewayProgramArguments = vi.fn(async () => ({
  programArguments: ["/bin/node", "cli", "gateway", "--port", "18798"],
}));
...
vi.mock("../infra/ports.js", () => ({
  ...
  formatPortDiagnostics: () => ["Port 18798 is already in use."],
}));
...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OPENHEDGEHOG_STATE_DIR: process.env.OPENHEDGEHOG_STATE_DIR,
    OPENHEDGEHOG_CONFIG_PATH: process.env.OPENHEDGEHOG_CONFIG_PATH,
    OPENHEDGEHOG_GATEWAY_PORT: process.env.OPENHEDGEHOG_GATEWAY_PORT,
    OPENHEDGEHOG_PROFILE: process.env.OPENHEDGEHOG_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENHEDGEHOG_STATE_DIR = "/tmp/openhedgehog-cli-state";
    process.env.OPENHEDGEHOG_CONFIG_PATH = "/tmp/openhedgehog-cli-state/openhedgehog.json";
    delete process.env.OPENHEDGEHOG_GATEWAY_PORT;
    delete process.env.OPENHEDGEHOG_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENHEDGEHOG_STATE_DIR !== undefined) {
      process.env.OPENHEDGEHOG_STATE_DIR = originalEnv.OPENHEDGEHOG_STATE_DIR;
    } else {
      delete process.env.OPENHEDGEHOG_STATE_DIR;
    }

    if (originalEnv.OPENHEDGEHOG_CONFIG_PATH !== undefined) {
      process.env.OPENHEDGEHOG_CONFIG_PATH = originalEnv.OPENHEDGEHOG_CONFIG_PATH;
    } else {
      delete process.env.OPENHEDGEHOG_CONFIG_PATH;
    }

    if (originalEnv.OPENHEDGEHOG_GATEWAY_PORT !== undefined) {
      process.env.OPENHEDGEHOG_GATEWAY_PORT = originalEnv.OPENHEDGEHOG_GATEWAY_PORT;
    } else {
      delete process.env.OPENHEDGEHOG_GATEWAY_PORT;
    }

    if (originalEnv.OPENHEDGEHOG_PROFILE !== undefined) {
      process.env.OPENHEDGEHOG_PROFILE = originalEnv.OPENHEDGEHOG_PROFILE;
    } else {
      delete process.env.OPENHEDGEHOG_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENHEDGEHOG_PROFILE: "dev",
        OPENHEDGEHOG_STATE_DIR: "/tmp/openhedgehog-daemon-state",
        OPENHEDGEHOG_CONFIG_PATH: "/tmp/openhedgehog-daemon-state/openhedgehog.json",
        OPENHEDGEHOG_GATEWAY_PORT: "19001",
      },
  ...
  it("installs the daemon when requested", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "18798"], {
      from: "user",
    });
  ...
  it("installs the daemon with json output", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "18798", "--json"], {
      from: "user",
    });
