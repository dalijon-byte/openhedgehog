...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openhedgehog.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenHedgehog hooks")
    ...
    .option("--hook-url <url>", "OpenHedgehog hook URL")
    .option("--hook-token <token>", "OpenHedgehog hook token")
