...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENHEDGEHOG_NO_RESPAWN: "1",
      OPENHEDGEHOG_STATE_DIR: stateDir,
      OPENHEDGEHOG_SKIP_CHANNELS: "1",
      OPENHEDGEHOG_SKIP_GMAIL_WATCHER: "1",
      OPENHEDGEHOG_SKIP_CRON: "1",
      OPENHEDGEHOG_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENHEDGEHOG_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openhedgehog-entry-bootstrap.mjs");
