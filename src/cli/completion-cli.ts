...
export async function installCompletion(shell: string, yes: boolean, binName = "openhedgehog") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c openhedgehog -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'openhedgehog gateway' and not 'openhedgehog other gateway', we need to check the sequence.
