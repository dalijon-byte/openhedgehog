...
${formatHelpExamples([
  ['openhedgehog agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openhedgehog agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openhedgehog agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openhedgehog agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openhedgehog agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openhedgehog agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openhedgehog.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openhedgehog.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'openhedgehog agents set-identity --agent main --name "OpenHedgehog" --emoji "🦞"',
    "Set name + emoji.",
  ],
  [
    "openhedgehog agents set-identity --agent main --avatar avatars/openhedgehog.png",
    "Set avatar path.",
  ],
  [
    "openhedgehog agents set-identity --workspace ~/.openhedgehog/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openhedgehog agents set-identity --identity-file ~/.openhedgehog/workspace/IDENTITY.md --agent main",
