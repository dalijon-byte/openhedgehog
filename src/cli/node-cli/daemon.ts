...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("openhedgehog node install"),
    formatCliCommand("openhedgehog node start"),
  ];
...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 18798;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(
        `Reinstall with: ${formatCliCommand("openhedgehog node install --force")}`,
      );
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OPENHEDGEHOG_LOG_PREFIX: baseEnv.OPENHEDGEHOG_LOG_PREFIX ?? "node",
