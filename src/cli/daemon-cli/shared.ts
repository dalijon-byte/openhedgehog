...
const SAFE_DAEMON_ENV_KEYS = [
  "OPENHEDGEHOG_PROFILE",
  "OPENHEDGEHOG_STATE_DIR",
  "OPENHEDGEHOG_CONFIG_PATH",
  "OPENHEDGEHOG_GATEWAY_PORT",
  "OPENHEDGEHOG_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("openhedgehog gateway install", env)}`,
    );
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.OPENHEDGEHOG_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("openhedgehog gateway install", env),
    formatCliCommand("openhedgehog gateway", env),
  ];
