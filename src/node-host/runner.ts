...
import { ensureOpenHedgehogCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.OPENHEDGEHOG_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENHEDGEHOG_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenHedgehogCliOnPath({ pathEnv: process.env.PATH ?? "" });
