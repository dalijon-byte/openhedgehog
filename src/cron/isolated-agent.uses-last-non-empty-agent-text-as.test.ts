...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openhedgehog-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".openhedgehog", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenHedgehogConfig> = {},
): OpenHedgehogConfig {
  const base: OpenHedgehogConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "openhedgehog"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".openhedgehog", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
