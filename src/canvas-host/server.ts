...
<title>OpenHedgehog Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.openhedgehogCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.openhedgehogCanvasA2UIAction &&
        typeof window.openhedgehogCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.openhedgehogSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("openhedgehog:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpenHedgehog node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openhedgehogSendUserAction === "function"
        ? window.openhedgehogSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENHEDGEHOG_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPENHEDGEHOG_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".openhedgehog", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpenHedgehog Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
