...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("openhedgehogCanvasA2UIAction");
    expect(out).toContain("openhedgehogSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("openhedgehogSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.openhedgehogA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("openhedgehog-a2ui-host");
      expect(html).toContain("openhedgehogCanvasA2UIAction");
      ...
      expect(js).toContain("openhedgehogA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
