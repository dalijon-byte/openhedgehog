  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "openhedgehog-update-preflight-"),
      );
    ...
    const doctorStep = await runStep(
      step(
        "openhedgehog doctor",
        managerScriptArgs(manager, "openhedgehog", ["doctor", "--non-interactive"]),
        gitRoot,
        { OPENHEDGEHOG_UPDATE_IN_PROGRESS: "1" },
      ),
    );
