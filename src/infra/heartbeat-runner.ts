...
function resolveActiveHoursTimezone(cfg: OpenHedgehogConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpenHedgehogConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpenHedgehogConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OpenHedgehogConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OpenHedgehogConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpenHedgehogConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpenHedgehogConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpenHedgehogConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
