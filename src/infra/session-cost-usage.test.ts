...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-cost-"));
    ...
    const originalState = process.env.OPENHEDGEHOG_STATE_DIR;
    process.env.OPENHEDGEHOG_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.OPENHEDGEHOG_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-cost-session-"));
