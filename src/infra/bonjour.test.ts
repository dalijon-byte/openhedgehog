...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.OPENHEDGEHOG_MDNS_HOSTNAME = "test-host";
    process.env.OPENHEDGEHOG_MDNS_HOSTNAME = "test-host";
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.gatewayPort).toBe("18798");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("openhedgehog (OpenHedgehog)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("openhedgehog");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("openhedgehog.local");
