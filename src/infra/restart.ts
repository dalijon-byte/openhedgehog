...
export function triggerOpenHedgehogRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENHEDGEHOG_SYSTEMD_UNIT,
        process.env.OPENHEDGEHOG_PROFILE,
      );
  ...
  const label =
    process.env.OPENHEDGEHOG_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENHEDGEHOG_PROFILE);
