...
type EnsureOpenHedgehogPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureOpenHedgehogPathOpts): string[] {
  ...
  // Bundled macOS app: `openhedgehog` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openhedgehog");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openhedgehog` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openhedgehog"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openhedgehog` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenHedgehogCliOnPath(opts: EnsureOpenHedgehogPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENHEDGEHOG_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENHEDGEHOG_PATH_BOOTSTRAPPED = "1";
