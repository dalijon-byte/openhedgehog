...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveOpenHedgehogPackageRoot } = await import("./openhedgehog-root.js");
    ...
    vi.mocked(resolveOpenHedgehogPackageRoot).mockResolvedValue("/opt/openhedgehog");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/openhedgehog",
      installKind: "package",
      packageManager: "npm",
