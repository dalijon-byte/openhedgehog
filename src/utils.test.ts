...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhedgehog-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "openhedgehog-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openhedgehog when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhedgehog-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openhedgehog")).toBe(path.resolve(os.homedir(), "openhedgehog"));
  });
