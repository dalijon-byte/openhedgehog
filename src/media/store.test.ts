...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENHEDGEHOG_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENHEDGEHOG_STATE_DIR = path.join(home, ".openhedgehog");
    ...
    await fs.mkdir(path.join(home, ".openhedgehog"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.openhedgehog${path.sep}media`);
