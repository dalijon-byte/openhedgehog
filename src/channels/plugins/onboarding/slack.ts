...
function setSlackDmPolicy(cfg: OpenHedgehogConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenHedgehog";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenHedgehog`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/openhedgehog",
          description: "Send a message to OpenHedgehog",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: OpenHedgehogConfig, allowFrom: string[]): OpenHedgehogConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
