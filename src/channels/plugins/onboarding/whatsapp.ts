...
function setWhatsAppDmPolicy(cfg: OpenHedgehogConfig, dmPolicy: DmPolicy): OpenHedgehogConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpenHedgehogConfig, allowFrom?: string[]): OpenHedgehogConfig {
...
async function detectWhatsAppLinked(cfg: OpenHedgehogConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OpenHedgehog can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpenHedgehog" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("openhedgehog channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
