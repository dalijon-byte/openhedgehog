...
describe("normalizeForwardedContext", () => {
  ...
  it("handles legacy forwards with signatures", () => {
    const ctx = normalizeForwardedContext({
      forward_from_chat: {
        title: "OpenHedgehog Updates",
        username: "openhedgehog",
        id: 99,
        type: "channel",
      },
    ...
    expect(ctx).not.toBeNull();
    expect(ctx?.from).toBe("OpenHedgehog Updates (Stan)");
    expect(ctx?.fromType).toBe("legacy_channel");
    expect(ctx?.fromId).toBe("99");
    expect(ctx?.fromUsername).toBe("openhedgehog");
    expect(ctx?.fromTitle).toBe("OpenHedgehog Updates");
