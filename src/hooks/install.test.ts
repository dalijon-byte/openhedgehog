...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openhedgehog-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@openhedgehog/zip-hooks",
        version: "0.0.1",
        openhedgehog: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openhedgehog/tar-hooks",
        version: "0.0.1",
        openhedgehog: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
