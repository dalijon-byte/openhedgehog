...
`;
    ...
    expect(parsed.openhedgehog.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openhedgehog.emoji).toBe("📝");
    expect(parsed.openhedgehog.events).toEqual(["command"]);
    expect(parsed.openhedgehog.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openhedgehog.install[0].kind).toBe("bundled");
  });
...
describe("resolveOpenHedgehogMetadata", () => {
  it("extracts openhedgehog metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenHedgehogMetadata(frontmatter);
  ...
  it("returns undefined when openhedgehog key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openhedgehog: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenHedgehog" },
            { id: "npm", kind: "npm", package: "@openhedgehog/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openhedgehog/hook");
  });
...
homepage: https://docs.openhedgehog.ai/hooks#session-memory
metadata:
  {
    "openhedgehog":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OpenHedgehog" }],
      },
  }
---
...
`;
    ...
    const openhedgehog = resolveOpenHedgehogMetadata(frontmatter);
    expect(openhedgehog).toBeDefined();
    expect(openhedgehog?.emoji).toBe("💾");
    expect(openhedgehog?.events).toEqual(["command:new"]);
    expect(openhedgehog?.requires?.config).toEqual(["workspace.dir"]);
    expect(openhedgehog?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openhedgehog = resolveOpenHedgehogMetadata(frontmatter);
    expect(openhedgehog?.emoji).toBe("disk");
    expect(openhedgehog?.events).toEqual(["command:new"]);
  });
});
