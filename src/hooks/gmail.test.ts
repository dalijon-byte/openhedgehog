...
import { type OpenHedgehogConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
...
const baseConfig = {
  hooks: {
    token: "hook-token",
    gmail: {
      account: "openhedgehog@gmail.com",
...
describe("gmail hook config", () => {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    if (result.ok) {
      expect(result.value.account).toBe("openhedgehog@gmail.com");
