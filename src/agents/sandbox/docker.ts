...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openhedgehog.sandbox=1");
  args.push("--label", `openhedgehog.sessionKey=${params.scopeKey}`);
  args.push("--label", `openhedgehog.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openhedgehog.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("openhedgehog.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openhedgehog sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openhedgehog sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openhedgehog sandbox recreate --all");
}
