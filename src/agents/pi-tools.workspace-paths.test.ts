...
import { createOpenHedgehogCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openhedgehog-ws-", async (workspaceDir) => {
      await withTempDir("openhedgehog-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenHedgehogCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openhedgehog-ws-", async (workspaceDir) => {
      await withTempDir("openhedgehog-override-", async (overrideDir) => {
        const tools = createOpenHedgehogCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openhedgehog-sandbox-", async (sandboxDir) => {
      await withTempDir("openhedgehog-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openhedgehog-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "openhedgehog-sandbox:bookworm-slim",
            containerPrefix: "openhedgehog-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createOpenHedgehogCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
