...
import { __testing, createOpenHedgehogCodingTools } from "./pi-tools.js";
...
describe("createOpenHedgehogCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenHedgehogTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenHedgehogCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openhedgehog-outside.txt");
