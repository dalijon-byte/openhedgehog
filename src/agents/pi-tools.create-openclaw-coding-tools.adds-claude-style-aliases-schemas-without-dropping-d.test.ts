...
const defaultTools = createOpenHedgehogCodingTools();

describe("createOpenHedgehogCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createOpenHedgehogCodingTools();
    ...
    try {
      ...
      const contents = "Hello from openhedgehog read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "openhedgehog-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "openhedgehog-workspace"),
    ...
    const tools = createOpenHedgehogCodingTools({ sandbox });
