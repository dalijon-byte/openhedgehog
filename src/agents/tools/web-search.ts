...
type WebSearchConfig = NonNullable<OpenHedgehogConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenHedgehogConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openhedgehog configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openhedgehog.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openhedgehog.ai",
      "X-Title": "OpenHedgehog Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
