...
/**
 * Cleanup suspended OpenHedgehog CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenHedgehogConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-cli-images-"));
  const paths: string[] = [];
