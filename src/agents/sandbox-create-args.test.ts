...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "openhedgehog-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "openhedgehog-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        "create",
        "--name",
        "openhedgehog-sbx-test",
        "--label",
        "openhedgehog.sandbox=1",
        "--label",
        "openhedgehog.sessionKey=main",
        "--label",
        "openhedgehog.createdAtMs=1700000000000",
        "--label",
        "openhedgehog.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "openhedgehog-sbx-no-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
