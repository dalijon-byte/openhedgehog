...
import type { OpenHedgehogConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: OpenHedgehogConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENHEDGEHOG_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openhedgehog security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpenHedgehogConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OPENHEDGEHOG_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: OpenHedgehogConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: OpenHedgehogConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: OpenHedgehogConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: OpenHedgehogConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpenHedgehogConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpenHedgehogConfig): boolean {
...
function isBrowserEnabled(cfg: OpenHedgehogConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpenHedgehogConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: OpenHedgehogConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
