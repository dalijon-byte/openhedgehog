import type { BrowserConfig, BrowserProfileConfig, OpenHedgehogConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENHEDGEHOG_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENHEDGEHOG_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openhedgehog" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENHEDGEHOG_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENHEDGEHOG_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenHedgehog browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openhedgehog/openhedgehog CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenHedgehogConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENHEDGEHOG_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openhedgehog";
