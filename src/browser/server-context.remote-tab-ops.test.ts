...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenHedgehogChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        openhedgehog: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("openhedgehog");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const openhedgehog = ctx.forProfile("openhedgehog");

    const opened = await openhedgehog.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("openhedgehog")?.lastTargetId).toBe("CREATED");
  });
});
