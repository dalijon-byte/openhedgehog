...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openhedgehog-profile-decorated");
}
 ...
 */
export function decorateOpenHedgehogProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENHEDGEHOG_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENHEDGEHOG_BROWSER_COLOR).toUpperCase();
