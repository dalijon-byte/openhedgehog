...
vi.mock("./chrome.js", () => ({
  resolveOpenHedgehogUserDataDir: vi.fn(() => "/tmp/openhedgehog-test/openhedgehog/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpenHedgehogUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openhedgehog-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOpenHedgehogUserDataDir).mockReturnValue(userDataDir);
