...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(typeof env.OPENHEDGEHOG_SERVICE_VERSION).toBe("string");
    expect(env.OPENHEDGEHOG_SYSTEMD_UNIT).toBe("openhedgehog-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENHEDGEHOG_LAUNCHD_LABEL).toBe("ai.openhedgehog.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    if (process.platform === "darwin") {
      expect(env.OPENHEDGEHOG_LAUNCHD_LABEL).toBe("ai.openhedgehog.work");
    }
  });
});
