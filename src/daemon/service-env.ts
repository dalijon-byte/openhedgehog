...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENHEDGEHOG_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.OPENHEDGEHOG_STATE_DIR;
  const configPath = env.OPENHEDGEHOG_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    OPENHEDGEHOG_PROFILE: profile,
    OPENHEDGEHOG_STATE_DIR: stateDir,
    OPENHEDGEHOG_CONFIG_PATH: configPath,
    OPENHEDGEHOG_GATEWAY_PORT: String(port),
    OPENHEDGEHOG_GATEWAY_TOKEN: token,
    OPENHEDGEHOG_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENHEDGEHOG_SYSTEMD_UNIT: systemdUnit,
    OPENHEDGEHOG_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENHEDGEHOG_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    OPENHEDGEHOG_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    OPENHEDGEHOG_STATE_DIR: stateDir,
    OPENHEDGEHOG_CONFIG_PATH: configPath,
    OPENHEDGEHOG_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENHEDGEHOG_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENHEDGEHOG_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENHEDGEHOG_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENHEDGEHOG_LOG_PREFIX: "node",
    OPENHEDGEHOG_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENHEDGEHOG_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENHEDGEHOG_SERVICE_VERSION: VERSION,
  };
}
