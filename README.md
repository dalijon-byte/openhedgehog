# 🦔 OpenHedgehog — Personal AI Assistant

<p align="center">
    <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/dalijon-byte/openhedgehog/main/docs/assets/openhedgehog-logo-text.png">
        <img src="https://raw.githubusercontent.com/dalijon-byte/openhedgehog/main/docs/assets/openhedgehog-logo-text.png" alt="OpenHedgehog" width="500">
    </picture>
</p>
...
<p align="center">
  <a href="https://github.com/dalijon-byte/openhedgehog/actions/workflows/ci.yml?branch=main"><img src="https://img.shields.io/github/actions/workflow/status/dalijon-byte/openhedgehog/ci.yml?branch=main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/dalijon-byte/openhedgehog/releases"><img src="https://img.shields.io/github/v/release/dalijon-byte/openhedgehog?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
  <a href="https://discord.gg/clawd"><img src="https://img.shields.io/discord/1456350064065904867?label=Discord&logo=discord&logoColor=white&color=5865F2&style=for-the-badge" alt="Discord"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

(this project is a fork of original OpenClaw- by the date 01.02.2026, from here https://github.com/openclaw/openclaw)
**OpenHedgehog** is a _personal AI assistant_ you run on your own devices.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, this is it.

[Website](https://openhedgehog.ai) · [Docs](https://docs.openhedgehog.ai) · [DeepWiki](https://deepwiki.com/openhedgehog/openhedgehog) · [Getting Started](https://docs.openhedgehog.ai/start/getting-started) · [Updating](https://docs.openhedgehog.ai/install/updating) · [Showcase](https://docs.openhedgehog.ai/start/showcase) · [FAQ](https://docs.openhedgehog.ai/start/faq) · [Wizard](https://docs.openhedgehog.ai/start/wizard) · [Nix](https://github.com/dalijon-byte/nix-hedgehogbot) · [Docker](https://docs.openhedgehog.ai/install/docker) · [Discord](https://discord.gg/clawd)

Preferred setup: run the onboarding wizard (`openhedgehog onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
Works with npm, pnpm, or bun.
New install? Start here: [Getting started](https://docs.openhedgehog.ai/start/getting-started)
...
Model note: while any model is supported, I strongly recommend **Anthropic Pro/Max (100/200) + Opus 4.5** for long‑context strength and better prompt‑injection resistance. See [Onboarding](https://docs.openhedgehog.ai/start/onboarding).

## Models (selection + auth)

- Models config + CLI: [Models](https://docs.openhedgehog.ai/concepts/models)
- Auth profile rotation (OAuth vs API keys) + fallbacks: [Model failover](https://docs.openhedgehog.ai/concepts/model-failover)
...
```bash
npm install -g openhedgehog@latest
# or: pnpm add -g openhedgehog@latest
...
Full beginner guide (auth, pairing, channels): [Getting started](https://docs.openhedgehog.ai/start/getting-started)
...
openhedgehog agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.openhedgehog.ai/install/updating) (and run `openhedgehog doctor`).
...
Switch channels (git + npm): `openhedgehog update --channel stable|beta|dev`.
Details: [Development channels](https://docs.openhedgehog.ai/install/development-channels).
...
```bash
git clone https://github.com/dalijon-byte/openhedgehog.git
cd openhedgehog
...
pnpm openhedgehog onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm openhedgehog ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `openhedgehog` binary.

## Security defaults (DM access)

OpenHedgehog connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.openhedgehog.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `openhedgehog pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `openhedgehog doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.openhedgehog.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.openhedgehog.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, BlueBubbles, Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.openhedgehog.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.openhedgehog.ai/nodes/voicewake) + [Talk Mode](https://docs.openhedgehog.ai/nodes/talk)** — always-on speech for macOS/iOS/Android with ElevenLabs.
- **[Live Canvas](https://docs.openhedgehog.ai/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.openhedgehog.ai/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.openhedgehog.ai/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.openhedgehog.ai/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.openhedgehog.ai/nodes).
- **[Onboarding](https://docs.openhedgehog.ai/start/wizard) + [skills](https://docs.openhedgehog.ai/tools/skills)** — wizard-driven setup with bundled/managed/workspace skills.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=dalijon-byte/openhedgehog&type=date&legend=top-left)](https://www.star-history.com/#dalijon-byte/openhedgehog&type=date&legend=top-left)
...
- **[Browser control](https://docs.openhedgehog.ai/tools/browser)** — openhedgehog‑managed Chrome/Chromium with CDP control.
...
- Workspace root: `~/.openhedgehog/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.openclaw/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.openhedgehog/openhedgehog.json` (model + defaults):
...
- Link the device: `pnpm openhedgehog channels login` (stores creds in `~/.openhedgehog/credentials`).
