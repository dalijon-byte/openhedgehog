...
import type { OpenHedgehogPluginApi, OpenHedgehogPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "openhedgehog-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OpenHedgehogPluginApi> = {}): OpenHedgehogPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenHedgehogPluginToolContext> = {}): OpenHedgehogPluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "openhedgehog-lobster-plugin-noisy-",
    );
  ...
  it("rejects invalid JSON from lobster", async () => {
    const { dir } = await writeFakeLobsterScript(
      ...
      "openhedgehog-lobster-plugin-bad-",
    );
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenHedgehogPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
