/**
 * Twitch channel plugin for OpenHedgehog.
 *
...
import { buildChannelConfigSchema } from "openhedgehog/plugin-sdk";
import type {
 ...
 * for OpenHedgehog. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenHedgehogConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenHedgehogConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenHedgehogConfig): boolean => {
