import type { PluginRuntime } from "openhedgehog/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENHEDGEHOG_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-nostr-"));
  process.env.OPENHEDGEHOG_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OPENHEDGEHOG_STATE_DIR?.trim() || env.OPENHEDGEHOG_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".openhedgehog");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENHEDGEHOG_STATE_DIR = previous;
    }
