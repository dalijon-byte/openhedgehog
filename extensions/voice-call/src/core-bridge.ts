...
function resolveOpenHedgehogRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENHEDGEHOG_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["openhedgehog"]) {
  ...
  throw new Error("Unable to resolve core root. Set OPENHEDGEHOG_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveOpenHedgehogRoot();
