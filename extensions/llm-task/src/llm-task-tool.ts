...
// NOTE: This extension is intended to be bundled with OpenHedgehog.
// When running from source (tests/dev), OpenHedgehog internals live under src/.
...
export function createLlmTaskTool(api: OpenHedgehogPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via openhedgehog.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhedgehog-llm-task-"));
