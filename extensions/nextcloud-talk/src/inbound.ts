import {
  ...
  type OpenHedgehogConfig,
  type RuntimeEnv,
} from "openhedgehog/plugin-sdk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as OpenHedgehogConfig,
    surface: CHANNEL_ID,
  });
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenHedgehogConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenHedgehogConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenHedgehogConfig);
