...
export function getA2uiPaths(env = process.env) {
  const srcDir = env.OPENHEDGEHOG_A2UI_SRC_DIR ?? path.join(repoRoot, "src", "canvas-host", "a2ui");
  const outDir = env.OPENHEDGEHOG_A2UI_OUT_DIR ?? path.join(repoRoot, "dist", "canvas-host", "a2ui");
...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(`${message} Skipping copy (OPENHEDGEHOG_A2UI_SKIP_MISSING=1).`);
      return;
    }
