...
TRASH
  ...
  start_gateway() {
    node dist/index.js gateway --port 18798 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
    GATEWAY_PID="$!"
  }
  ...
  wait_for_gateway() {
    for _ in $(seq 1 20); do
      if node --input-type=module -e "
        ...
        const socket = net.createConnection({ host: '127.0.0.1', port: 18798 });
      ...
      if [ -f /tmp/gateway-e2e.log ] && grep -E -q "listening on ws://[^ ]+:18798" /tmp/gateway-e2e.log; then
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    node dist/index.js onboard --non-interactive --accept-risk \
      --mode remote \
      --remote-url ws://gateway.local:18798 \
...
if (cfg?.gateway?.remote?.url !== "ws://gateway.local:18798") {
    ...
    cat > "$HOME/.openclaw/openclaw.json" <<'"'"'JSON'"'"'
{
  ...
  "gateway": {
    "mode": "remote",
    "remote": { "url": "ws://old.example:18798", "token": "old-token" }
  }
}
JSON
