...
import type { OpenHedgehogConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openhedgehog-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openhedgehog-bin-sherpa-");
      const modelDir = await makeTempDir("openhedgehog-sherpa-model-");
      ...
      const cfg: OpenHedgehogConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openhedgehog-bin-whispercpp-");
      const modelDir = await makeTempDir("openhedgehog-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openhedgehog-bin-gemini-");
      ...
      const cfg: OpenHedgehogConfig = { tools: { media: { image: {} } } };
