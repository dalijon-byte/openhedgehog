...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENHEDGEHOG_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPENHEDGEHOG_STATE_DIR = path.join(base, ".openhedgehog");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openhedgehog-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openhedgehog", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
