...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhedgehog-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openhedgehog");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENHEDGEHOG_SKIP_CHANNELS: "1",
          OPENHEDGEHOG_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENHEDGEHOG_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPENHEDGEHOG_GATEWAY_TOKEN: inst.gatewayToken,
        OPENHEDGEHOG_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENHEDGEHOG_GATEWAY_PORT: String(gwA.port),
          OPENHEDGEHOG_GATEWAY_TOKEN: gwA.gatewayToken,
          OPENHEDGEHOG_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENHEDGEHOG_GATEWAY_PORT: String(gwB.port),
          OPENHEDGEHOG_GATEWAY_TOKEN: gwB.gatewayToken,
          OPENHEDGEHOG_GATEWAY_PASSWORD: "",
        }),
